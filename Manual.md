...
### 6.5 Hierarchy のオブジェクト管理機能 (フォルダ化・ロック)
シーンが複雑になりオブジェクトが増えてきた場合、以下の機能を使って Hierarchy を整理・保護することができます。これらはすべてシリアライズ（JSON保存）に対応しています。

1. **フォルダ機能 (Folder)**
   - Hierarchy 上の空白部分を **右クリック** し、「**Create Folder**」を選択すると、空のフォルダ（`isFolder_ = true` の GameObject）が作成されます。
   - フォルダには専用のアイコン（📂）が付き、他のオブジェクトを見やすくグループ化できます。
   - フォルダオブジェクトは Scene View 上ではギズモ（移動ハンドル等）が非表示になり、誤操作を防ぎます。
2. **ロック機能 (Lock)**
   - Hierarchy の各項目の右端にある **南京錠アイコン**（🔒/🔓）をクリックすると、そのオブジェクトの編集をロックできます。
   - ロックされたオブジェクトは以下の操作が制限されます：
