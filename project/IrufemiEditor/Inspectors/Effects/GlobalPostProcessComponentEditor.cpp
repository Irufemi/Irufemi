...
void GlobalPostProcessComponentEditor::DrawFloatProperty(const char* label, float& value, float defaultValue, float minVal, float maxVal, EditorActionManager* actionManager) {
    ImGui::TableNextRow();
    ...
    ComponentUIHelpers::CheckUndoRedoDrag(actionManager, &value, std::function<void(const float&)>([&value](const float& v){ value = v; }));

    std::string resetId = std::string("##Reset") + label;
    ComponentUIHelpers::DrawPropertyResetButton(resetId.c_str(), value != defaultValue, [&]() {
        float oldVal = value;
        ComponentUIHelpers::PushInstantUndo(actionManager, oldVal, defaultValue, std::function<void(const float&)>([&value](const float& v){ value = v; }));
    });
}

void GlobalPostProcessComponentEditor::DrawBoolProperty(const char* label, bool& value, bool defaultValue, EditorActionManager* actionManager) {
    ImGui::TableNextRow();
    ComponentUIHelpers::DrawPropertyLabel(label);
...
void GlobalPostProcessComponentEditor::Draw(Component* component, EditorActionManager* actionManager) {
    ...
    for (size_t i = 0; i < overrides.size(); ++i) {
        ...
        std::string icon = "";
        if (setting->GetMode() == PostProcessMode::Bloom) icon = ICON_FA_SUN " ";
        else if (setting->GetMode() == PostProcessMode::ToneMapping) icon = ICON_FA_PALETTE " ";
        else if (setting->GetMode() == PostProcessMode::Vignette) icon = ICON_FA_CAMERA " ";

        headerName = icon + headerName + "##" + std::to_string(i);

        bool open = ImGui::CollapsingHeader(headerName.c_str(), ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_AllowOverlap);
    ...
    if (ImGui::Button(ICON_FA_PLUS " Add Effect", ImVec2(-1, 0))) {
        ...
        if (selectedEffect == 0) mode = PostProcessMode::Bloom;
        else if (selectedEffect == 1) mode = PostProcessMode::ToneMapping;
        else if (selectedEffect == 2) mode = PostProcessMode::Vignette;
