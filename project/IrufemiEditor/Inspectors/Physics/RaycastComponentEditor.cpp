    ...
    if (headerOpen) {
        if (ComponentUIHelpers::BeginPropertyTable("RaycastTable")) {
            ...
            ComponentUIHelpers::DrawPropertyResetButton(
                "##OriginReset",
                comp->localOffset_.x != 0.0f || comp->localOffset_.y != 0.0f || comp->localOffset_.z != 0.0f, [&]() {
                    Irufemi::Vector3 oldO = comp->localOffset_;
                    ComponentUIHelpers::PushInstantUndo(actionManager, oldO, Irufemi::Vector3{0, 0, 0},
                                                        &comp->localOffset_);
                });
            ...
            ComponentUIHelpers::DrawPropertyResetButton(
                "##DirReset",
                comp->localDirection_.x != 0.0f || comp->localDirection_.y != 0.0f || comp->localDirection_.z != 1.0f,
                [&]() {
                    Irufemi::Vector3 oldD = comp->localDirection_;
                    ComponentUIHelpers::PushInstantUndo(actionManager, oldD, Irufemi::Vector3{0, 0, 1},
                                                        &comp->localDirection_);
                });
            ...
            if (treeOpen) {
                ...
                auto* cm = comp->GetGameObject() ? comp->GetGameObject()->GetScene()->GetEngine()->GetCollisionManager()
                                                 : nullptr;
            ...
            if (comp->hitInfo_.isHit) {
                ...
                ImGui::Text("Pt: (%.2f, %.2f, %.2f)", comp->hitInfo_.hitPoint.x, comp->hitInfo_.hitPoint.y,
                            comp->hitInfo_.hitPoint.z);
            } else {
