...
struct PlayerStateAttack final : IPlayerState {
	...
	void Enter(Player& player) override {
		...
		player.attackTransform_.translate = player.transform_.translate;
		player.attackTransform_.rotate = player.transform_.rotate;
	}
	...
	void Update(Player& player) override {
		...
		case Phase::Charge: {
			...
			player.transform_.scale.z = Lerp(1.0f, 0.3f, EaseOutSine(t));
			player.transform_.scale.y = Lerp(1.0f, 1.6f, EaseOutSine(t));
			if (timer_ >= 2) {
				phase_ = Phase::Tackle;
				timer_ = 0;
			}
		...
		case Phase::Tackle: {
			...
			player.transform_.scale.z = Lerp(0.3f, 1.3f, EaseOutSine(t));
			player.transform_.scale.y = Lerp(1.6f, 0.7f, EaseInSine(t));
			if (timer_ >= 10) {
		...
		case Phase::Aftereffect: {
			...
			float t = static_cast<float>(timer_) / 2.0f;
			player.transform_.scale.z = Lerp(1.3f, 1.0f, EaseOutSine(t));
			player.transform_.scale.y = Lerp(0.7f, 1.0f, EaseOutSine(t));
			if (timer_ >= 2) {
				player.transform_.scale.z = 1.0f;
				player.transform_.scale.y = 1.0f;
