...
void Enemy::Update(float deltaTime, const Vector3& playerPos) {
    ...
    case EnemyState::Idle: {
        ...
        if (actionTimer_ <= 0.0f) {
            ...
            // 実行不能な行動は重みを 0 にする
            if (!canWall) { weights.wall = 0.0f; }
            if (!canBullet) { weights.bullet = 0.0f; }
            if (!canBurrow) { weights.burrow = 0.0f; }
            if (!canDash) { weights.dash = 0.0f; }
            ...
            case 3: { // 突進攻撃
                if (canDash) {
                    ...
                    float len = std::sqrt(
                        dashDirection_.x * dashDirection_.x +
    ...
    if (enemyWall_) {
        if (state_ == EnemyState::DashForward) {
            int hitWallIndex =
                enemyWall_->CheckCollisionCircle(transform_.translate, enemyBodyRadius_);
            if (hitWallIndex >= 0) {
