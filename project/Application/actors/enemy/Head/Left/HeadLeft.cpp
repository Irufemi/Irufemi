...
void HeadLeft::Draw(IrufemiEngine* engine) {
...
void HeadLeft::SetTransform(const Transform& transform, const Vector3* drawWorldPos) {
  if (!isBlownAway_) {
...
void HeadLeft::OnDestroyed(const Vector3& attackDir, float blowSpeed) {
...
void HeadLeft::ApplyDamage(int damage) {
  hp_ -= damage;
  if (hp_ < 0) {
    hp_ = 0;
  }
