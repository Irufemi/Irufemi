...
void NormalBehavior::Update([[maybe_unused]] Particle &particle,
                            [[maybe_unused]] float deltaTime) {
  // 何もしない
}
void NormalBehavior::MakeNewParticle(Particle &particle,
...
void NormalBehavior::Debug([[maybe_unused]] Emitter *emitter,
                           [[maybe_unused]] DebugUI *ui,
...
void AccelerationFieldBehavior::MakeNewParticle(Particle &particle,
                                                std::mt19937 &randomEngine,
                                                const Emitter &emitter) {
  std::uniform_real_distribution<float> distTime(1.0f, 3.0f);
  particle.startScale = emitter.startScale;
...
void AccelerationFieldBehavior::Debug([[maybe_unused]] Emitter *emitter,
                                      DebugUI *ui,
...
void HitEffectBehavior::Update([[maybe_unused]] Particle &particle,
                               [[maybe_unused]] float deltaTime) {
  // 何もしない
}
void HitEffectBehavior::MakeNewParticle(Particle &particle,
                                        std::mt19937 &randomEngine,
                                        const Emitter &emitter) {
...
void HitEffectBehavior::Debug([[maybe_unused]] Emitter *emitter,
                              [[maybe_unused]] DebugUI *ui,
                              [[maybe_unused]] ParticleSystem *particleSystem) {
  // HitEffect固有のUIはなし
}
...
void ExplosionBehavior::MakeNewParticle(Particle &particle,
                                        std::mt19937 &randomEngine,
                                        const Emitter &emitter) {
  std::uniform_real_distribution<float> distRotate(-std::numbers::pi_v<float>,
                                                   std::numbers::pi_v<float>);
  ...
  particle.startScale = {distScale(randomEngine), distScale(randomEngine),
                         1.0f};
...
void MuzzleSmokeBehavior::Update(Particle &particle,
                                 [[maybe_unused]] float deltaTime) {
  // 速度の減衰(空気抵抗)
...
void MuzzleSmokeBehavior::MakeNewParticle(Particle &particle,
                                          std::mt19937 &randomEngine,
                                          const Emitter &emitter) {
...
void MuzzleSmokeBehavior::Debug(
...
void MuzzleFlashBehavior::Initialize(Emitter *emitter) {
  ...
  emitter->startColor = {1.0f, 0.7f, 0.1f,
                         1.0f}; // 鮮やかなオレンジ（通常ブレンド用に正規化）
...
void MuzzleFlashBehavior::MakeNewParticle(Particle &particle,
                                          std::mt19937 &randomEngine,
                                          const Emitter &emitter) {
  std::uniform_real_distribution<float> distType(0.0f, 1.0f);
  std::uniform_real_distribution<float> distTime(0.06f,
                                                 0.13f); // 残像感のある寿命
  std::uniform_real_distribution<float> distRotate(-std::numbers::pi_v<float>,
                                                   std::numbers::pi_v<float>);
  ...
  if (typeRoll < 0.3f) {
    ...
    particle.startScale = {distWidth(randomEngine), distLength(randomEngine),
                           1.0f};
    // 速度をラインの方向に合わせる（より放射状に見せる）
    float angle = distRotate(randomEngine);
    particle.transform.rotate.z = angle;
    float speed = 20.0f;
    particle.velocity = {std::sin(angle) * speed, -std::cos(angle) * speed,
                         0.0f};
  }
...
void MuzzleFlashBehavior::Debug(
    [[maybe_unused]] Emitter *emitter, [[maybe_unused]] DebugUI *ui,
    [[maybe_unused]] ParticleSystem *particleSystem) {
#ifdef USE_IMGUI
...
void MissileFireBehavior::Initialize(Emitter *emitter) {
  ...
  emitter->startColor = {4.0f, 1.5f, 0.2f,
                         1.0f}; // 赤・オレンジ系の強い炎カラー
...
void MissileFireBehavior::MakeNewParticle(Particle &particle,
                                          std::mt19937 &randomEngine,
                                          const Emitter &emitter) {
...
void MissileFireBehavior::Debug(
    [[maybe_unused]] Emitter *emitter, [[maybe_unused]] DebugUI *ui,
    [[maybe_unused]] ParticleSystem *particleSystem) {}
...
void MissileSmokeBehavior::MakeNewParticle(Particle &particle,
                                           std::mt19937 &randomEngine,
                                           const Emitter &emitter) {
...
void MissileSmokeBehavior::Debug(
    [[maybe_unused]] Emitter *emitter, [[maybe_unused]] DebugUI *ui,
    [[maybe_unused]] ParticleSystem *particleSystem) {}
...
void EjectionMistBehavior::Initialize(Emitter *emitter) {
  ...
  emitter->startColor = {0.9f, 0.9f, 0.9f,
                         0.12f}; // 不透明度を 0.25 -> 0.12 に低下（より薄く）
  emitter->endColor = {1.0f, 1.0f, 1.0f, 0.0f};
  emitter->colorMode = ParticleColorMode::kNone;
}

void BulletTrailBehavior::Update(Particle &particle,
                                 [[maybe_unused]] float deltaTime) {
  // その場に留まる
  particle.velocity *= 0.0f;
}
void BulletTrailBehavior::MakeNewParticle(Particle &particle,
                                          std::mt19937 &randomEngine,
                                          const Emitter &emitter) {
  std::uniform_real_distribution<float> distTime(0.15f,
                                                 0.25f); // わずかに寿命を伸ばす
  std::uniform_real_distribution<float> distScale(0.8f, 1.2f);
  particle.startScale = emitter.startScale * distScale(randomEngine);
  particle.endScale = emitter.endScale;
...
void BulletTrailBehavior::Debug(
    [[maybe_unused]] Emitter *emitter, [[maybe_unused]] DebugUI *ui,
    [[maybe_unused]] ParticleSystem *particleSystem) {}

void EjectionMistBehavior::Update([[maybe_unused]] Particle &particle,
                                  [[maybe_unused]] float deltaTime) {
  // 空気抵抗でゆっくり止まる
  particle.velocity *= 0.9f;
}
void EjectionMistBehavior::MakeNewParticle(Particle &particle,
                                           std::mt19937 &randomEngine,
                                           const Emitter &emitter) {
...
void EjectionMistBehavior::Debug(
    [[maybe_unused]] Emitter *emitter, [[maybe_unused]] DebugUI *ui,
    [[maybe_unused]] ParticleSystem *particleSystem) {}

// GroundSmokeBehavior
void GroundSmokeBehavior::Initialize(Emitter *emitter) {
  emitter->count = 20;
  emitter->area = {0.2f, 0.2f, 0.2f};
  emitter->velocityMin = {-0.5f, -0.5f, -0.5f};
  emitter->velocityMax = {0.5f, 0.5f, 0.5f};
...
void GroundSmokeBehavior::Update(Particle &particle, float deltaTime) {
  particle.velocity *= 0.98f; // 空気抵抗で減速

  // 地面に到達した場合の滞留処理
  if (particle.transform.translate.y <= groundHeight_) {
    particle.transform.translate.y =
        groundHeight_; // 地面にめり込まないように固定

    if (particle.velocity.y < 0.0f) {
      particle.velocity.y = 0.0f; // 下向きの速度をなくす
      // 地面での滞留感（横への広がり）を強調する
      particle.velocity.x *= 1.5f;
      particle.velocity.z *= 1.5f;
    }
  }
}
void GroundSmokeBehavior::MakeNewParticle(Particle &particle,
                                          std::mt19937 &randomEngine,
                                          const Emitter &emitter) {
  std::uniform_real_distribution<float> distTime(0.5f, 1.2f); // 長生き
  std::uniform_real_distribution<float> distScale(0.5f, 2.0f);
...
void GroundSmokeBehavior::Debug(
    [[maybe_unused]] Emitter *emitter, [[maybe_unused]] DebugUI *ui,
    [[maybe_unused]] ParticleSystem *particleSystem) {}
