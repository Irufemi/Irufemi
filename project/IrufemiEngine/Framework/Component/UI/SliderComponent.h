...
public:
    ...
    std::string GetComponentName() const override { return "SliderComponent"; }
     ...
     */
    void SetOnValueChangedCallback(std::function<void(float)> callback) { onValueChangedCallback_ = callback; }

    /**
     * @brief ハンドル（ツマミ）となるGameObjectのIDを設定
     */
    void SetHandleObjectID(int id) { handleObjectID_ = id; }
