...
void HitEffectBehavior::MakeNewParticle(Particle& particle, std::mt19937& randomEngine, const Emitter& emitter) {
	...
	std::uniform_real_distribution<float> distScale(3.0f, 7.0f);// もとは(0.4f, 1.5f)
	particle.startScale = { 1.0f, distScale(randomEngine), 1.0f };// もとは{ 0.1f, distScale(randomEngine), 1.0f }
