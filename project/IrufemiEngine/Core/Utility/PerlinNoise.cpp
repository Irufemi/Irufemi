...
float PerlinNoise::Noise1D(float x) const {
    return (Noise(x) * 2.0f) -
           1.0f; // Note: standard Perlin often ranges slightly differently, but mapping [0,1] to [-1,1] is usually
                 // fine. Wait, Perlin output is actually roughly [-sqrt(3/4), sqrt(3/4)].
